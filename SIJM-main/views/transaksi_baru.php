...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['simpan_transaksi'])) {
    // Ambil object Database Connection secara eksternal untuk melacak id transaksi terakhir
    $db = (new Database())->getConnection();
    
    if ($trxCtrl->prosesTransaksi($_POST, $_SESSION['user_id'])) {
        // Ambil ID Transaksi yang baru saja masuk untuk dialihkan ke halaman detail struk
        $id_terakhir = $db->lastInsertId();
        
        echo "<script>
            alert('Transaksi berhasil disimpan!');
            window.location.href = 'detail_transaksi.php?id=" . $id_terakhir . "';
        </script>";
        exit;
    }
}
...
<body class="bg-gray-100 flex flex-col md:flex-row min-h-screen">
    ...
    <script>
        ...
        // Validasi pengisian nama pelanggan sebelum submit
